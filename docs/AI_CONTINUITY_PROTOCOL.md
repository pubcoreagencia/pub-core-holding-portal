...
## Required lifecycle
1. Read repository context before changing the project.
2. Implement and validate the change.
3. Update relevant context, architecture, status, or handoff documentation.
4. Commit the complete evolution to Git.
5. Push or open a pull request according to repository workflow.
6. Leave a clear handoff when work remains incomplete.

A task is not durably complete merely because it worked inside an AI session or local workspace.
